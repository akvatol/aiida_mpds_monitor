...
Before creating the `.7z`, the monitor applies both checks: every process below the parent workchain must have completed successfully (`is_finished_ok`), and the collected contents must contain exactly three calculation folders with one `OUTPUT` file in each. Failing either check prevents archive creation and upload.
